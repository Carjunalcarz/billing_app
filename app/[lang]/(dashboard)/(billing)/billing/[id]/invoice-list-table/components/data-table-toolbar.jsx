...
export function DataTableToolbar({ table }) {
  ...
  // State to store the selected number of rows
  const [pageSize, setPageSize] = useState(table.getState().pagination.pageSize);

  // Function to update the number of rows displayed
  const handlePageSizeChange = (value) => {
    const newSize = parseInt(value, 10);
    setPageSize(newSize);
    table.setPageSize(newSize); // Update the table's pagination
  };
          ...
          <Select value={String(pageSize)} onValueChange={handlePageSizeChange}>
            ...
            <SelectContent className="w-20 min-w-[80px]">
              {Array.from({ length: 10 }, (_, index) => {
                const number = (index + 1) * 10; // Generates values: 10, 20, 30, ..., 100
                return (
