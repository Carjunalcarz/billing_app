...
const CreateBilling = ({ id }) => {
  ...
  const [localId, setLocalId] = useState(id); // Store ID locally

  useEffect(() => {
    if (id) {
      setLocalId(id); // Update local state when id is available
    }
  }, [id]);
  useEffect(() => {
    if (localId && status === "authenticated") {
      fetchSubscribers(localId);
    }
  }, [localId, session, status]);
  const dueDate = subscriptions?.endDate
    ? new Date(subscriptions.endDate).toLocaleDateString("en-US", {
        year: "numeric",
        month: "long",
        day: "numeric",
      })
  ...
  const fetchSubscribers = async (id1) => {
    // console.log("TOKEN : ", session.user.accessToken);
    if (session?.user?.accessToken) {
      const token = session.user.accessToken;

      try {
        const response = await fetch(`${apiUrl}/api/subscriptions/${id1}`, {
          method: "GET",
          headers: {
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-5">
                      <div className="text-lg font-semibold text-default-900">
                        Billing To:
                        {`${subscriptions?.client?.firstName}  ${subscriptions?.client?.lastName}`}
                      </div>
                      <div className="text-lg font-medium text-default-800 mt-1">
                        {/* Prantik Chakraborty */}
                      </div>
                      <div className="text-base text-default-600 mt-3">
                        {`${subscriptions?.client?.street}. ${subscriptions?.client?.barangay}, ${subscriptions?.client?.city}, ${subscriptions?.client?.zipCode} `}
                      </div>
                    </div>
                  </div>
                  <div className="flex-none md:text-end">
                    <div className="text-lg font-semibold text-default-900">
                      Subscriber Acount ID :
                    </div>
                    <div className="mt-1.5 text-lg font-medium text-default-600">
                      {subscriptions._id} <br />
                    </div>
                    <div className="mt-4 text-base text-default-600">
                      Alcarz Internet Zone
                      <br />
                      Soriano , Cabadbaran City <br />
                      #09104422786 <br />
                    </div>
                    <div className="mt-8">
